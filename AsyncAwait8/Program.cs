...
namespace AsyncAwait8
{
    class Program
    {
        ...
        {
            CookDinner();
            Console.WriteLine("Hello World!"+ GetThreadId());
            Console.ReadKey();
        }
        ...
        /// 做饭
        /// </summary>
        public async static void CookDinner()
        {
            ...
            sw.Start();//开始计时
            //与2对比 添加await
            string result = await CommandBuySalt();
            sw.Stop();//结束计时


            //string result1 = await CommandBuySalt();
           ...
           // Console.WriteLine("用了盐炒的菜就是好吃【{0}】，线程Id为：{1}", result1, GetThreadId());
